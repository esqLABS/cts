...
test_that("for_single_sim", {
  ...
  expect_snapshot(suppressWarnings(pretty_pk(my_ddi, molecule_name = "Levonorgestrel 1", pk_parameter = c("C_max", "C_tEnd"))))
  expect_snapshot(suppressWarnings(compare_pk(my_ddi)))
...
test_that("for pop simulation", {
  ...
  expect_snapshot(suppressWarnings(pretty_pk(lng, molecule_name = "Levonorgestrel 1", pk_parameter = c("AUC_tEnd", "C_tEnd"))))
  expect_snapshot(suppressWarnings(compare_pk(lng)))
  expect_snapshot(suppressWarnings(compare_pk(lng, reference_simulation = "Bayer Study 19604 - control Day 1")))
})
