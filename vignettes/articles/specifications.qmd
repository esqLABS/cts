...
-   Git repository with the entire history

# Concept

## Inputs

As inputs, the users select two compound model snapshot files (from the
OSP model library or provided by the user). These compound model
snapshots are standardized `.json` files generated from PKSim and have
...
-   A set of essential PK statistics and figures that describe the DDI
    simulation results,
-   A simulation project file (.pkml) that can be opened in PKSim.
...
-   An internet connection to install packages and dependencies from
    GitHub and CRAN,
...
-   Import compound snapshots from a json file path or a URL,
...
-   The codebase must be versioned, revertible, and traceable,
-   The development environment must be reproducible.

# User Experience

::: callout-note
The pseudocode snippets below are illustrative and do not represent the
...
```
### Export the DDI simulation as a snapshot and/or project
...
-   Orchestrating the different components to provide a complete
    workflow,
-   Interacting with the OSP model library and retrieving the available
    models,
...
interact with the `{rSharp}` package, which will be necessary to
interact with the PKSim Core. Moreover, the features that need to be
implemented for this project can benefit the whole `{ospsuite}` package
and its users.

Thus, the `{ospsuite}` package will be extended in order to provide:

-   A convenient interface with PKSimCore for running simulations and
    exporting results from snapshot files,
-   A convenient interface with PKSimCore to export project file
    `.pksim5` files from snapshot files.

### `{rSharp}`

The `{rSharp}` already contains all the necessary tools to act as a
bridge between R and the C# core of PKSim.
...
-   Exporting simulation files (.pkml) from a snapshot file.
...
Development will start with R version 4.4.0 and is set as the minimum
required version.
...
There is no predefined list or restriction in using other R packages, so
any required packages will be added as a dependency. Although, keeping
the number of dependencies as low as possible will be an important
consideration.
...
-   Function references describing the function's purpose, arguments,
    and return values using the roxygen2 format,
...
Functions and classes that are not exported will be documented using
...
and extensible for other features like advanced model parameterization
or administration protocols.

## Graphical User Interface

The `{cts}` package is first thought of as a standalone programming
interface concentrating the logic and scientific basis of the project.
This both enables code-based usage of the package and the possibility to
...
| Review input snapshot parameters               |     2 |          1 |           0 |              2 |
...
dependencies. Even if they are planned sequentially, some tasks will be
...
-   Review input snapshot parameters
...
The project will be carried out using the Agile approach for software
development. This implies:

-   Iterative and incremental implementations,
...
The project is planned to be completed by the 31st of October 2024.
